...
def _read_data_description_table(ms_path_str: str) -> tuple[dict[int, tuple[int, int]], list[str]]:
    ...
    with open_table(ms_path_str + "/DATA_DESCRIPTION") as tb:
        ...
        for dd_id, (spw_id, pol_id) in enumerate(zip(spw_ids, pol_ids)):
...
def run_spectral_window_list(ms_path: str) -> dict:
    ...
    for dd_id, (spw_id, pol_id) in dd_to_spw_pol.items():
    ...
    with open_msmd(ms_str) as msmd:
        ...
        for spw_id in range(n_spw):
            ...
            if pol_id is not None and pol_id in pol_id_to_codes:
                ...
                labels = []
                corr_field = field(None, flag="UNAVAILABLE",
                                    note=f"No DATA_DESCRIPTION row maps to SpW {spw_id}")
            ...
            record: dict = {
                ...
                "band_name":         field(band_name, flag=band_flag,
                                           note=None if telescope else "Telescope unknown"),
            }
            ...
            if not width_uniform:
                ...
                record["channel_width_hz"] = field(
                    round(chan_width_hz, 2), flag="PARTIAL",
...
def run_correlator_config(ms_path: str) -> dict:
    ...
    data = {
        "dump_time_s":          field(round(dump_time_s, 3) if dump_time_s else None,
                                       flag=dump_flag, note=dump_note),
        ...
        "full_stokes":          field(
            ...
            note=(
                ...
                if full_stokes else
                "Not full-Stokes — polarisation imaging limited"
            ),
        ),
