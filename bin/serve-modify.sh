...
if ! command -v pixi >/dev/null 2>&1; then
    cat >&2 <<EOF
[ms-modify] pixi is not on PATH, so this server cannot start.
...
if ! pixi run --manifest-path "$MANIFEST" python -c "import casatools" 2>/dev/null; then
    echo "[ms-modify] Installing CASA tools (first run only)..." >&2
...
exec pixi run --manifest-path "$MANIFEST" serve-modify
